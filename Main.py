...
h = st.sidebar.number_input('Wall Thickness h (mm)', value=600)
d = st.sidebar.number_input('Depth d (mm)', value=540)
...
shear_spacing_H = st.sidebar.number_input('Horizontal Shear Reinforcement Spacing (mm)', value=200)
shear_spacing_V = st.sidebar.number_input('Vertical Shear Reinforcement Spacing (mm)', value=200)

# 주철근
main_diameter = st.sidebar.selectbox('Main Reinforcement Diameter (mm)', list(rebar_areas.keys()))
selected_main_area = rebar_areas[main_diameter]
st.sidebar.write(f"Selected Main Reinforcement Area: {selected_main_area} mm²")

main_spacing = st.sidebar.number_input('Main Reinforcement Spacing (mm)', value=200)

# 작용 외력 산정 표시 및 입력 받기
st.header('Input for Calculating Applied External Forces')
L2 = st.number_input('L2 (m)', value=4.0)  # 띠장-벽체사이 거리
L1 = st.number_input('L1 (m)', value=2.5)  # 타설높이
...
st.header('Calculation of External Forces Acting on Cantilever Wall')
Mu = 1.8 * M1 + 1.2 * M2 
Vu = 1.8 * V1 + 1.2 * V2
...
st.header('Calculation and Review of Member Strength and Structural Stability')

st.subheader('Shear Strength Review')
...
st.subheader('Flexural Strength Review')
